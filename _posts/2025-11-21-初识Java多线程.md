...
**注意这里的线程的状态机是java层面定义的，操作系统层面存在的与之不同的是操作系统没有blocked状态**，事实上java的blocked状态是一个上层的定义，其在os底层也是waiting状态，java针对synchronized锁定义了该状态，其在解锁时底层也是用了Os的unpark函数。而对于reentrantLock加锁等待时的状态就是waiting了
